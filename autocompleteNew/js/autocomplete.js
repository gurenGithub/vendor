...
var HaierJS = window.HaierJS || {};
	...
	$.fn.autoComplete = function(options){
		...
		$html.on('click', 'li', function(e){
			...
			if(o.onChange){
				o.onChange($(this).text())
			}
			hide();
		})
		o.$input.on('click.showAutocomplete', function(e){
			e.preventDefault();
			e.stopPropagation();
			show();
		})

		o.$input.on('keyup.autocomplete', function (e) {
		    if (o.onKeyup) {
		        o.onKeyup();
		    }
			if(e.keyCode == 40) {
			...
			if(typeof o.items == 'function') {
				...
				update($.grep(o.items, function(s){
					return s.toLowerCase().indexOf(o.$input.val().toLowerCase()) > -1;
				}), true)
			}
